...
const connectWallet = async ()=>{
    if (window.ethereum) {
        const provider = new (0, _ethers.ethers).providers.Web3Provider(window.ethereum);
        ...
        document.getElementById('balance').textContent = `Balance: ${(0, _ethers.ethers).utils.formatEther(balance)} ETH`;
        return {
            provider,
            signer,
            walletAddress
...
const sendEth = async ()=>{
    ...
    if (!(0, _ethers.ethers).utils.isAddress(recipient)) {
        alert("Invalid recipient address");
        return;
    }
    const tx = await signer.sendTransaction({
        to: recipient,
        value: (0, _ethers.ethers).utils.parseEther(amount)
    });
...
const castVote = async (proposal)=>{
    ...
    const contract = new (0, _ethers.ethers).Contract(contractAddress, abi, signer);
    try {
...
},{"ethers":"hdHML"}],"hdHML":[function(require,module,exports,__globalThis) {
/**
...
},{"./ethers.js":false,"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gkKU3":[function(require,module,exports,__globalThis) {
...
},{}]},["dIizP","bB7Pu"], "bB7Pu", "parcelRequire94c2")
